...
| `/browser [connect\|disconnect\|status]` | Manage local Chrome CDP connection. `connect` attaches browser tools to a running Chrome instance (default discovery endpoint: `http://127.0.0.1:9222`). `disconnect` detaches. `status` shows current connection. Auto-launches Chrome with a dedicated user-data-dir and local-only debugging address if no debugger is detected. |
